...
i = 0;
figure(5)
sgtitle("Gaussian Noise , Average Filter ")
subplot(2,2,1)
imshow(picGauss)
title("Noisy Picture")

figure(6)
sgtitle("Gaussian Noise , Average Filter , Histogarm ")
subplot(2,2,1)
hist1 = pichist(picGauss);
title("histogram :  gaussian Noisy Picture")
ylabel("number of repeat")
...
for n = [3 5 7] %filter size
    ...
    picAveFiltered = uint8(conv2(picGauss,avefilter,'same'));
...
figure(7)
sgtitle("Gaussian Noise , Median Filter ")
subplot(2,2,1)
imshow(picGauss)
title("Noisy Picture")
figure(8)
sgtitle("Gaussian Noise , Median Filter , Histogarm ")
subplot(2,2,1)
hist1 = pichist(picGauss);
title("histogram : gaussian Noisy Picture")
ylabel("number of repeat")
...
for n = [3 5 7]
    i = i+1;
    medfilter = ones(n);
    picMedFiltered = uint8(ordfilt2(picGauss,round(n^2/2),medfilter)); 
    figure(7)
    subplot(2,2,i+1)
